...
import { Card, Eyebrow, OutlineButton, AssetMark, StatusBadge } from '../components/ui'
import {
  marketRows,
  orders,
  watchlistSymbols,
...
export function Dashboard() {
  ...
  const watchlist = marketRows.filter((r) => watchlistSymbols.includes(r.symbol))
  ...
  return (
    <>
      ...
      <div className="mb-4 grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard label="Portfolio value" value="$12,840.56" detail="+$284.20 (2.26%) today" icon={Wallet} />
        <StatCard label="Available cash" value="$4,250.00" detail="Ready to invest" icon={Banknote} tone="teal" />
        <StatCard label="Total return" value="+$1,840.56" detail="+16.72% all time" icon={TrendingUp} tone="violet" />
      </div>
        ...
        <Card>
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <h2 className="text-[17px] font-semibold tracking-tight">Performance</h2>
            </div>
          ...
          <div className="my-5 flex items-center gap-4">
            <strong className="text-[26px] font-bold tracking-tight">$12,840.56</strong>
        ...
        <Card>
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <h2 className="text-[17px] font-semibold tracking-tight">Recent orders</h2>
            </div>
            <button
            ...
            {orders.slice(0, 4).map((order) => (
              ...
              >
                ...
                <div className="flex flex-1 flex-col">
                  <b className="text-xs text-ink">{order.side} {order.symbol}</b>
                  <small className="text-[10px] text-muted">{order.shares} shares · {order.date}</small>
                </div>
                <div className="flex flex-col items-end gap-1">
                  <span className="text-xs font-semibold text-ink">{usd(order.total)}</span>
        ...
        <Card>
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <h2 className="text-[17px] font-semibold tracking-tight">Watchlist</h2>
            </div>
            <button
          ...
          <MarketTable rows={watchlist} actionLabel="View" onAction={() => navigate('/markets')} />
        </Card>
      </div>
    </>
  )
